---
description: 포트폴리오 콘텐츠를 src/data/resume.ts 단일 파일로 구성합니다 (선행: /project-init)
argument-hint: "<이력서 PDF 경로 (선택)>"
---

포트폴리오에 표시될 모든 콘텐츠를 `src/data/resume.ts` 단일 파일로 관리합니다.
...
- `src/data/resume.ts` — 경력, 프로젝트, 기술, SEO를 담은 **단일 소스**
...
- **단일 소스**: 콘텐츠 수정은 오직 `src/data/resume.ts` 하나만 변경
