...
주어진 PDF 파일을 읽고 `portfolio/src/lib/data.ts`를 실제 내용으로 재구성합니다.

## Steps

1. **PDF 읽기**
   - `ARGUMENTS`로 전달된 경로의 PDF를 Read 툴로 읽으세요
...
3. **data.ts 업데이트**
   - `portfolio/src/lib/data.ts`를 Read 툴로 먼저 읽은 뒤
   ...
   - 프로젝트 이미지(`image`, `screenshots`)는 PDF에 정보가 없으면 /resource 에 있는 png 파일을 사용.
